...
```
buildroot官网下载`https://buildroot.org/download.html`
...
```
<<<<<<< HEAD
使用交叉编译工具来测试一下`/root/buildroot-2019.02.8/output/host/bin/mipsel-linux-gcc helloworld.c -o helloworld.out -static`
```c
#include <stdio.h>
...
>>>>>>> 1fe4e49bb0ac4cc8912aea4ad2771a20d28094a6
```
运行
```
...
- User mode(使用者模式) :启动的是程序
- System mode(系统模式) : 模拟的是整个计算机系统

在使用者模式下QEMU-MIPS运行大端序程序,QEMU-MIPSEL运行小端序程序,使用的命令大概如下
```
cp $(which qemu-mipsel) ./
./qemu-mipsel hello "just test"	#hello程序,参数是just test
```
如果需要动态库
```
cp $(which qemu-mipsel) ./
chroot . ./qemu-mips hello "just test"
```
系统模式下命令: `qemu-system-mips [options] [disk_image]` disk_image 是一个原始的的IDE硬盘镜像

#### 关于/etc/network/interfaces 的说明
详细的请参考官方文档`https://www.debian.org/doc/manuals/debian-reference/ch05.en.html`
1. 有线网络:系统已经内置了一个有线网络配置文件。
```bash
 cubie@Cubian:~# sudo cat /etc/network/interfaces 
 auto eth0				#表示在启动时自动配置网卡接口 eth0
 allow-hotplug eth0			#表示当Linux内核检测到物理网卡接口 eth0,allow-hotplug 语句指示 ifup 激活此接口
 iface eth0 inet dhcp			#iface 语句指示ifup使用DHCP配置此接口.
```
...
你仍然可以通过以下命令手工激活并配置有线网络`ifup eth0`
